...
function speak(text, force, msgEl) {
  return new Promise(function(resolve) {
    ...
    // Cached TTS ready — play instantly
    if (msgEl && msgEl._ttsUrl) {
      play(msgEl._ttsUrl);
      return;
    }

    // First play or replay with no cache: use browser voice immediately,
    // while fetching server TTS in background for subsequent replays
    fallbackSpeak(text, function() {});
    if (msgEl) { cacheTTS(msgEl, text); }
    else { fetchTTSBlob(text).then(function(b) { return URL.createObjectURL(b); }); }
    resolve();
  });
}
