...
function speak(text, force) {
  return new Promise((resolve) => {
    ...
    // If force (user clicked replay), hard cancel
    if (force && speakBusy) {
      window.speechSynthesis.cancel();
      speakBusy = false; if (speakPending) { speakPending(); speakPending = null; }
    }
    // If auto-speaking overlaps, cancel current and speak new
    if (!force && speakBusy) {
      window.speechSynthesis.cancel();
      speakBusy = false; if (speakPending) { speakPending(); speakPending = null; }
    }
    ...
    const v = findVoice(preset.lang, preset.gender);
    if (v) u.voice = v;
    u.lang = preset.lang;
    const rate = (preset.lang === 'en-GB' ? 0.92 : 0.95) * speechRate;
    u.rate = rate;
    // Lower pitch at high speed to reduce TTS engine glitching
    u.pitch = speechRate > 1.15 ? 0.85 : speechRate > 1.0 ? 0.93 : 1.0;
    const done = () => { speakBusy = false; speakPending = null; resolve(); };
    u.onend = done;
    u.onerror = done;
    window.speechSynthesis.speak(u);
  });
...
if (window.speechSynthesis) {
  ...
  window.speechSynthesis.onvoiceschanged = () => { window.speechSynthesis.getVoices(); prewarmTTS(); };
}
...
function onMessage(msg) {
  switch (msg.type) {
    ...
    case 'reply_ipa': showIPA(msg.text); break;
    case 'assistant_reply': onAssistantReply(msg); break;
...
function onScenarioStarted(msg) {
  ...
  if (!scenarioInitialized) {
    ...
    addBubble('assistant', msg.opening);
    prewarmTTS(); speak(msg.opening);
  } else {
...
function onReplyDone(msg) {
  ...
  if (el) {
    ...
    el.title = '点击重读';
    el.addEventListener('click', () => speak(clean || final, true));
    var act = document.createElement('div');
    act.className = 'msg-actions';
    el.appendChild(act);
    addStarButton(act, clean || final);
    addTranslateButton(act, clean || final);
...
function addBubble(role, text) {
  ...
  // Action buttons (star + translate)
  var actions = document.createElement('div');
  actions.className = 'msg-actions';
  div.appendChild(actions);
  if (role === 'assistant') {
    div.title = '点击重读';
    div.addEventListener('click', () => speak(text, true));
...
function renderScenarios(list) {
  ...
  scenarioGrid.querySelectorAll('.scenario-card').forEach(c =>
    c.addEventListener('click', () => startScenario(c.dataset.id))
  );
}
